...
// Same theme->trim sync as the local page: a theme card carrying its natural
// page (data-trim) sets the trim until the trim is chosen by hand. The card
// picker and spec panels survive the serverless adaptation, so this is the
// local logic verbatim (the polling submit handler below is what differs).
...
def _page() -> str:
    ...
    # subn + a match check: a silent no-match would ship the local page's
    # markup (e.g. a PDF picker, or the polling script) on a host that can't
    # honor it — fail loudly if the adaptations drift out of sync with PAGE.
